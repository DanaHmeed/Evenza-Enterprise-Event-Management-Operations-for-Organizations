...
import { t, Field } from "@/components/dashboard/OrganizerUI";
...
/* ─── inp() ─────────────────────────────────────────────────────────────
   Always use this instead of spreading a base style + borderColor.
   Mixing the `border` shorthand with `borderColor` longhand causes React
   to warn on every re-render. Using only longhands avoids the conflict.
──────────────────────────────────────────────────────────────────────── */
function inp(
  error?: string,
...
export default function CreateEventPage() {
  ...
  const validate = () => {
    ...
    if (form.startDate && form.endDate && new Date(form.endDate) <= new Date(form.startDate))
      errs.endDate = "Must be after start";
    if (form.startDate && form.registrationDeadline && new Date(form.registrationDeadline) >= new Date(form.startDate))
      errs.registrationDeadline = "Must be before start";
    if (form.eventType === "PAID" && (!form.price || parseFloat(form.price) <= 0))
      errs.price = "Required for paid events";
    if (form.isOnline && !form.meetingLink.trim()) errs.meetingLink = "Required for online events";
  ...
  const handleSubmit = async (status: "DRAFT" | "PUBLISHED") => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({
          message: status === "PUBLISHED" ? "Event published!" : "Draft saved",
        ...
        setToast({ message: "Failed to create event", description: d.error || "Something went wrong.", variant: "error" });
      }
    } catch {
      ...
      <div style={{ fontFamily: "Rubik, sans-serif", minHeight: "100vh", background: "#f7f7f4" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: t.textMuted }}>
            ...
            <span style={{ color: t.text, fontWeight: 600 }}>Create Event</span>
          </div>
        ...
        {/* ── Two-column layout ── */}
        <div style={{
          maxWidth: 1240, margin: "0 auto", padding: "28px 24px 60px",
            ...
            <div style={card}>
              ...
              <Field label="Summary" sublabel="Short description shown on event cards">
                <input
                  ...
                  placeholder="A brief one-liner for your event"
                  style={inp()}
                  maxLength={300}
                />
              </Field>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
                <Field label="Registration Deadline *" error={errors.registrationDeadline}>
                  <input
                    ...
                    style={inp(errors.registrationDeadline)}
                  />
                </Field>
                  ...
                  <Field label="Ticket Price *" error={errors.price}>
                    <div style={{ position: "relative" }}>
                      <span style={{
                        ...
                        color: t.textFaint, fontSize: 13, pointerEvents: "none",
                      }}>$</span>
                      <input
                        type="number"
                        value={form.price}
                        ...
                        style={inp(errors.price, { paddingLeft: 28 })}
                      />
                    </div>
                  </Field>
